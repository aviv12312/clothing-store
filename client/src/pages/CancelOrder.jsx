  ...
  return (
    <div className="min-h-screen bg-white pt-28 pb-20 px-6" dir="rtl">
      <div className="mx-auto max-w-xl">
        ...
        <h1 className="mt-3 font-['Noto_Serif'] text-4xl text-[#121211]">ביטול עסקה</h1>
        <p className="mt-2 font-['Manrope'] text-sm text-[#9A958C]">
          בהתאם לחוק הגנת הצרכן ניתן לבטל עסקה תוך 14 ימים מיום קבלת המוצר.
        </p>

        {status === 'success' ? (
          <div className="mt-10 bg-[#FBFAF7] p-8 text-center">
            <p className="text-3xl mb-4">✦</p>
            <h2 className="font-['Noto_Serif'] text-xl text-[#121211]">הבקשה התקבלה</h2>
            <p className="mt-3 font-['Manrope'] text-sm text-[#6E6A62]">{msg}</p>
            <p className="mt-2 font-['Manrope'] text-xs text-[#9A958C]">
              אישור נשלח לכתובת המייל שלך. נחזור אליך תוך 2 ימי עסקים.
            </p>
            <button onClick={() => navigate('/profile')} className="mt-6 font-['Manrope'] text-xs uppercase tracking-[0.2rem] border border-[#121211] px-6 py-3 hover:bg-[#121211] hover:text-white transition-colors">
              חזרה לפרופיל
            </button>
          </div>
        ) : (
          <form onSubmit={handleSubmit} className="mt-10 space-y-6">
            ...
            {/* בחירת הזמנה */}
            <div>
              <label className="block font-['Manrope'] text-xs uppercase tracking-[0.2rem] text-[#6E6A62] mb-2">
                בחר הזמנה לביטול
              </label>
              {orders.length === 0 ? (
                <p className="font-['Manrope'] text-sm text-[#9A958C] bg-[#FBFAF7] p-4">
                ...
                <select
                  ...
                  required
                  className="w-full border border-[#DEDAD2] px-4 py-3 font-['Manrope'] text-sm focus:outline-none focus:border-[#121211]"
                >
                ...
                {REASONS.map((r) => (
                  <label key={r} className="flex items-center gap-3 cursor-pointer">
                    <input
                      ...
                      className="accent-[#121211]"
                      required
                    />
                    <span className="font-['Manrope'] text-sm text-[#6E6A62]">{r}</span>
                  </label>
                ))}
              </div>
              {reason === 'אחר' && (
                <textarea
                  ...
                  required
                  rows={3}
                  className="mt-3 w-full border border-[#DEDAD2] px-4 py-3 font-['Manrope'] text-sm focus:outline-none focus:border-[#121211] resize-none"
                />
              )}
            </div>

            {/* הצהרה */}
            <div className="bg-[#FBFAF7] p-4 font-['Manrope'] text-xs text-[#6E6A62] leading-relaxed">
            ...
            <button
              ...
              className="w-full bg-[#121211] text-white font-['Manrope'] text-xs uppercase tracking-[0.2rem] py-4 hover:bg-[#6E6A62] transition-colors disabled:opacity-50"
            >
